...
*/
const receive = new XMLHttpRequest()
receive.addEventListener('readystatechange', () => {
  const isRequestOk = receive.readyState === 4 && receive.status === 200
  const isRequestNotOk = receive.readyState === 4
  if(isRequestOk){
...
receive.open('GET', 'https://pokeapi.co/api/v2/pokemon/pikachu');
// receive.send();
...
const meters = [5,9,15,28] 

meters.forEach( meter => person.distance(meter))
// console.log(person.metros, person.andar)
...
person.getString = () => {
  const { name, sobrenome, idade, altura, metros, peso} = person
  const pluralOrSingular = getPluralOrSingular(person.idade, 'ano', 'anos')
  const distancePerson = getPluralOrSingular(person.metros,'metro', 'metros')
  const femaleOrMale = person.sexo === 'masculino' ? 'o' : 'a'

  return `Oi. Eu sou ${femaleOrMale} ${name} ${sobrenome}, tenho ${idade} ${pluralOrSingular}, ${altura} ${distancePerson} de altura, peso ${peso} quilos e, só hoje, eu já caminhei ${metros} ${distancePerson}.
  `
}
console.info(person.getString());
...
const retornaBoolean = value => Boolean(value);
const falsyValues = [false, 0, '', null, undefined, NaN,document.all]
const truthyValues = [true, '0', ()=>{}, {}, [], -1, 'false']
const logFalsyValues = falsyValue => console.log(retornaBoolean(falsyValue))
const logTruthyValues = truthyValue => console.log(retornaBoolean(truthyValue))
...
const retornoBook = getBook => {
  const nameBook = {
    'harry poter': {
      'A quantidade de páginas': 985,
      'Autor': "Valeria",
      'editora': 'leiturinha'
    },
    'a menina que roubava livros':{
      'A quantidade de páginas': 200,
      'Autor': "José",
      'editora': 'calabouço'
    },
    'Mosquito':{
      'A quantidade de páginas': 741,
      'Autor': "Maria",
      'editora': 'cultura'
    }
  }

  return nameBook[getBook] || nameBook // utilizou curto circuito
}

// console.log(retornoBook('a menina que roubava livros'))