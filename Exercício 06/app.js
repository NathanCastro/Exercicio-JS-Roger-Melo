...
for (let i = 0; i < numbers.length; i++) {
  ...
  const isNumber =number >= 11 && number <= 90;
  if ( isNumber  ) {
...
for (let i = 0; i < crazyArray.length; i++) {
  if (typeof crazyArray[i] === 'boolean' ) {
    ...
    // console.log(`O crazyArray tem ${arrayBoolean[i]} booleans`); 
...
const evenNumberString = arrayImpar.join(', ').replace(', 3', ' e 3')
const oddNumberString = arrayPar.join(', ').replace(', 5', ' e 5')
console.log(`"Numeros ímpares: ${evenNumberString}. 
Números pares: ${oddNumberString}."`);