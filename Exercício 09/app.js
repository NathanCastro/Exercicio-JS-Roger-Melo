...
const criarFuncao = string2 => string2.length
//console.log(criarFuncao('garrafa'));
...
const letraMinuscula = frase => frase.toLowerCase()
//console.log(letraMinuscula("CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO"));
...
const exer04 = (character, string) => string.indexOf(character);
//console.log(exer04('h', 'lasanha'));
...
*/
const arrayBoolean = (item, array) => array.includes(item)
//console.log(arrayBoolean('copo', ['talher', 'faca', 'garfo']));
...
const joinArray = (array1, array2) => array1.concat(array2)
//console.log(joinArray([1,2,3,4], [5,6,7,8]));
/*
  07
...
const ultimoArray = array => array.pop();
// console.log(ultimoArray(['twitter', 'instagram', 'snapchat', 'tiktok']));
//essa maneira de cima, eu retorno o item excluido

//Resolução do professor
const removeLastItem = array => {
  array.pop() //se eu deixar só assim, vai me retornar o item retirado
  return array // vai retornar o array, sem o último item  
}

// console.log(removeLastItem([0,9,8]));
...
const varNull = value => value === null;
...
const invocaCall = callback => {
  const retorno = 'Nathan';
...
*/
// const invocaFuntion = valueArgument => {
//   const valueTriplo = 33 * 3;

//   valueArgument(valueTriplo);
// }
// invocaFuntion((numberTriplo) => {
//   console.log(numberTriplo);
// })
...
const numbers = [1, 2, 3];
numbers.forEach((substiNumber,index,arr) => {
  // console.log(`"O ${index + 1}º item do array [${arr.join(', ')}] é ${substiNumber}."`);
...
review.forEach(reviewText => {
  paragraphs += `<div>${reviewText}</div>`
})
...
const like = (pessoasLikaram = []) => {   

  switch (pessoasLikaram.length) {
    case 0:
      return 'ninguém curtiu'
      
    
    case 1:
      return `${pessoasLikaram[0]} curtiu isso`;
      
    
    case 2:
      return `${pessoasLikaram[0]}, ${pessoasLikaram[1]} curtiu isso`;
      
    
    case 3:
      return `${pessoasLikaram[0]}, ${pessoasLikaram[1]}, ${pessoasLikaram[2]} curtiram isso`;
      
    
    default:
      return `${pessoasLikaram[0]}, ${pessoasLikaram[1]}, e mais ${pessoasLikaram.length - 2} curtiram isso`;
  }
};
console.log(like(['lacraia', 'cafetao', 'bandido', ]));
