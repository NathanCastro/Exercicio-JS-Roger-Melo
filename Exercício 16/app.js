...
const valueH2 = document.querySelector('h2')
const div = document.querySelector('div')
const elementsInsideDiv = Array.from(div.children)
const coordenada = document.querySelector('.egg');
const btn = document.querySelector('button');

elementsInsideDiv.forEach(element => {
  element.addEventListener('click', event => {
    event.stopPropagation()
    valueH2.textContent = `Clicou no ${event.target.tagName.toLowerCase()} filho da div.`
  })
})

div.addEventListener('click', () => {
  valueH2.textContent = 'Clicou na div.'
})
...
elementsInsideDiv.forEach(itemFilho => {
  itemFilho.addEventListener('click', event => {
    const resultClick = event.target;
    const finalValor = resultClick.innerHTML.toLowerCase();
    c(`'Clicou no ${finalValor}, filho da div.'`)
})
...
*/
  // valueH2.addEventListener('copy', () => {
  //   c('texto copiado')
  // }) ve o q rolou, não pegou
...
coordenada.addEventListener('mousemove', event => {
  const mouseEventX = event.offsetX
  const mouseYEvent = event.offsetY

  coordenada.textContent = `"Eixo X: ${mouseEventX} | Eixo Y: ${mouseYEvent}"`
...
btn.addEventListener('click', () => {  
  ...
  coordenada.style.background = 'lightgoldenrodyellow'
})
...
const resultado = people.forEach(items => {
  if(items.profession === 'Front-end developer'){
   c('O array people contém, no mínimo, um(a) Front-end developer')
  }
})