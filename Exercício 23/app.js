...
const newOrder = people.map(({ firstName, lastName, score }) => ({firstName, lastName, score}))
.sort((numberA, numberB) => (numberA.score - numberB.score))
// c(newOrder)
...
const newAnimais = animals.filter(({length}) => length === 3);
// c(newAnimais)
...
const qtdAnimal = animals.map(({length}) => length)
// c(qtdAnimal)
...
const newFriends = friends.filter(({nearMe}) =>nearMe).map(({name}) => name);
// c(newFriends)
...
const sunImpar = numbers.filter(numeros => numeros % 2)
...
const some = data.filter(item => item.country !== 'China')
